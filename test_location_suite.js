...
const idTests = [
  ...
  { in: 'Gorontalo', expC: 'Indonesia', expP: 'Gorontalo' },
...
const mockCards = {
  ...
  // Kasus dari laporan user
  u_id4: { asal: 'Kembangan', location: parseLocation('Kembangan') },
  u_id5: { asal: 'Aceh', location: parseLocation('Aceh') },
  u_id6: { asal: 'DKI Jakarta', location: parseLocation('DKI Jakarta') },
  u_id7: { asal: 'Sumatra', location: parseLocation('Sumatra') },
  u_id8: { asal: 'Jambi', location: parseLocation('Jambi') }
};
...
assert(mapResult.totalValidLocations === 14, `Total valid locations: ${mapResult.totalValidLocations} / 14`);
...
assert(regionNames.includes('DKI Jakarta'), 'Hierarki Region mencakup provinsi DKI Jakarta 🇮🇩');
assert(regionNames.includes('Aceh'), 'Hierarki Region mencakup provinsi Aceh 🇮🇩');

// Uji metadata region di membermap embed
assert(mapResult.locMetadata['Kembangan']?.region === 'DKI Jakarta', 'Kembangan memiliki region DKI Jakarta di membermap');
assert(mapResult.locMetadata['Aceh']?.region === 'Provinsi', 'Aceh memiliki region Provinsi di membermap');
assert(mapResult.locMetadata['DKI Jakarta']?.region === 'Provinsi', 'DKI Jakarta memiliki region Provinsi di membermap');
assert(mapResult.locMetadata['Sumatera']?.region === 'Pulau', 'Sumatera memiliki region Pulau di membermap');
assert(mapResult.locMetadata['Kota Jambi']?.region === 'Jambi', 'Kota Jambi memiliki region Jambi di membermap');
